...
import REST_API.RequestObjects.UpdateEmail;
import REST_API.RequestObjects.UpdatePassword;
import REST_API.config.JwtFilter;
...
import REST_API.config.JwtProvider;
...
//    }
    ...
    public String authorization(@RequestBody User user) throws UserNoExistException {
        try{
            ...
            if(user2 != null){
                String token = jwtProvider.generateToken(user2.getEmail(), user2.getPassword(), user2.getId());
                return token;
            }
        } catch (Exception e){
            return null;
